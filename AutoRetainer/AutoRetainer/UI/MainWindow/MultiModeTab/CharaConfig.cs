    ...
    public static void Draw(OfflineCharacterData data, bool isRetainer)
    {
        ...
        else
        {
            b = b.Section("远航探索").Widget(() =>
            {
                ...
                ImGuiComponents.HelpMarker("此设置类似于全局选项，但应用于单个角色。启用后，AutoRetainer 将在登录角色之前等待所有远航探索返回。如果您因其他原因已经登录，它仍然会重新派遣已完成的潜水艇/飞空艇，除非全局设置「即使已登录也等待」也同时开启。");
            });
        }
        ...
        .Widget(() =>
        {
            ...
            ImGui.Indent();
            ImGuiEx.Checkbox("为召唤铃传送...", ref data.TeleportOptionsOverride.Retainers);
            ImGui.Indent();
            ...
            ImGui.Unindent();
            ImGuiEx.Checkbox("为潜水艇/飞空艇传送至部队房屋", ref data.TeleportOptionsOverride.Deployables);
            ImGui.Unindent(); 
        }).Draw();
