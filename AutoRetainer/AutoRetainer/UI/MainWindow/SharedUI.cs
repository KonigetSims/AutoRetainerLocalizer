...
internal static class SharedUI
{
    internal static void DrawLockout(OfflineCharacterData data)
    {
        if(data.IsLockedOut())
        {
            ...
            ImGuiEx.Tooltip("此角色位于您已暂时禁用的数据中心。请前往配置界面取消禁用。");
            ImGui.SameLine();
        }
    }
    ...
    internal static void DrawServiceAccSelector(OfflineCharacterData data)
    {
        ...
        if(ImGui.BeginCombo("##Service Account Selection", $"服务账户 {data.ServiceAccount + 1}", ImGuiComboFlags.HeightLarge))
        {
            for(var i = 1; i <= 10; i++)
            {
                if(ImGui.Selectable($"服务账户 {i}"))
                {
    ...
    internal static void DrawExcludeReset(OfflineCharacterData data)
    {
        ...
        .Widget(() =>
        {
            ...
            ImGuiComponents.HelpMarker("角色的已保存数据将被清除但不会排除该角色。当您再次登录此角色时，角色数据将重新生成。");

                if(ImGui.Button("清除部队数据"))
            {
                data.ClearFCData();
            }
            ImGuiComponents.HelpMarker("部队数据、飞空艇和潜水艇将从该角色移除。数据将在可用时重新生成。");
        }).Draw();
    }
}
